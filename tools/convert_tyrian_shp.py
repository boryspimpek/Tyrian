"""
Wyodrebnia sprite'y z pliku tyrian.shp do osobnych plikow PNG z przezroczystością.
...
Wynik:  folder Tyrian\tyrian21\extracted_tiles\extracted_tyrian_shp/ z plikami {bank}_{index:04d}.png.
       Czarne piksele są zamieniane na przezroczyste.
"""
...
def save_png(w, h, pixels, pal_path, name):
    if not pixels or len(pixels) == 0: return

    # Wczytaj paletę jako płaską listę RGB
    palette_flat = []
    if os.path.exists(pal_path):
        ...
        for i in range(256):
            palette_flat.extend([i, i, i])

    # Uzupełnij paletę do 256 kolorów (768 wartości)
    while len(palette_flat) < 768:
        palette_flat.extend([0, 0, 0])

    # Stwórz obraz w trybie paletowym
    img = Image.new('P', (w, h))
    img.putdata(pixels)
    ...
    # Zamień ciemne piksele na przezroczyste
    datas = img.getdata()
    new_data = []
    for item in datas:
        # Jeśli piksel jest ciemny (RGB < 30), ustaw alpha na 0
    ...
    png_name = name.replace('.bmp', '.png')
    img.save(png_name, optimize=True)
...
def extract_banks_0_6(f, bank_idx, bank_start, out_dir, pal_path):
    ...
    for i in range(count):
        ...
        if any(pixels):
            save_png(w, h, pixels, pal_path, f"{out_dir}/{name}_{i:04d}.bmp")
...
def extract_banks_7_11(f, bank_idx, bank_start, bank_end, out_dir, pal_path):
    ...
    for i in range(n_sprites):
        ...
        if pixels and any(pixels):
            save_png(w, h, pixels, pal_path, f"{out_dir}/{name}_{i+1:04d}.bmp")
...
def extract_tyrian_shp(file_path, pal_path):
    ...
    count = len([x for x in os.listdir(out_dir) if x.endswith('.png')])
